...
const projectsDirectory = path__WEBPACK_IMPORTED_MODULE_1___default().join(process.cwd(), "projects");
function getSortedProjectsData() {
    // Get file names under /posts
    const fileNames = fs__WEBPACK_IMPORTED_MODULE_0___default().readdirSync(projectsDirectory);
    const allProejctsData = fileNames.map((fileName)=>{
    ...
    // Sort posts by date
    return allProejctsData.sort(({ date: a  }, { date: b  })=>{
        if (a < b) {
            return 1;
        } else if (a > b) {
            return -1;
